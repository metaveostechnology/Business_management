...
<body class="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display">
    <div class="relative flex h-screen w-full overflow-hidden">
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col transform lg:translate-x-0 lg:static closed">
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-2">
                ...
                <a href="{{ route('employee_self.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('employee_self.dashboard') ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800' }} transition-all font-semibold">
                    <span class="material-symbols-outlined">dashboard</span>
                    <span class="text-sm">Dashboard</span>
                </a>
                <a href="{{ route('employee_self.leaves') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('employee_self.leaves') ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800' }} transition-colors font-medium">
                ...
                <a href="{{ route('employee_self.profile') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('employee_self.profile') ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800' }} transition-colors font-medium">
                ...
                <div class="flex items-center gap-6">
                    ...
                    <div class="relative">
                        ...
                        <div id="profile-dropdown" class="absolute right-0 mt-3 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 py-2 hidden z-50">
                            ...
                            <a href="{{ route('employee_self.profile') }}" class="flex items-center gap-3 px-4 py-2 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
    ...
    <script>
        ...
        function getAuthToken() { return localStorage.getItem('branch_user_token'); }
        function removeAuthToken() { localStorage.removeItem('branch_user_token'); localStorage.removeItem('branch_user'); }

        async function logout() {
            try {
                await fetch(apiBaseUrl + '/employee/logout', {
                    method: 'POST',
                    headers: {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const userRaw = localStorage.getItem('branch_user');
            if (userRaw) {
                try {
