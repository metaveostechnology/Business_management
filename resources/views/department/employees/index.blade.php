    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <h5 class="card-title mb-0 flex-grow-1">Department Employees</h5>
...
<script>
    ...
    async function openEditModal(slug) {
        try {
            const data = await apiRequest(`/dept/employees/${slug}`);
            const emp = data.data;
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const endpoint = isEdit ? `/api/dept/employees/${slug}` : '/api/dept/employees';
    ...
    async function deleteEmployee(slug) {
        ...
        if (result.isConfirmed) {
            try {
                await apiRequest(`/dept/employees/${slug}`, 'DELETE');
