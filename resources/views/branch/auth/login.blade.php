...
@section('content')
<div class="card">
    <div class="auth-logo">Branch Portal</div>
    <div class="auth-sub">Sign in as Branch Admin</div>
    ...
    <form id="loginForm">
        <div class="form-group">
            ...
            <input type="email" id="email" class="form-control" required placeholder="admin@branch.com">
        </div>
...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(apiBaseUrl + '/branch-admin/login', {
            method: 'POST',
            headers: {
        ...
        if (response.ok && data.status) {
            ...
            setTimeout(() => {
                window.location.href = "{{ route('branch.dashboard') }}";
            }, 1000);
        } else {
